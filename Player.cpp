...
{
	if (m_xmf3Position.x <= -120) {
		m_xmf3Position.x = -120;
	}
	...
	if (m_xmf3Position.z > 298 * 100.0f) {
...
CAirplanePlayer::CAirplanePlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	...
	CGameObject *pGameObject = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Models/OldCar.bin");
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	switch (nNewCameraMode)
	{
		case FIRST_PERSON_CAMERA:
			...
			break;
		case THIRD_PERSON_CAMERA:
			SetFriction(50.0f);			// 20.5f
			SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));
			SetMaxVelocityXZ(150.0f);	// 25.5f
			SetMaxVelocityY(40.0f);
